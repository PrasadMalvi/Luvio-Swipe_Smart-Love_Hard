...
const SignUp = ({ navigation , setIsLoggedIn}) => {
  ...
  const dispatch = useDispatch();
    ...
    const validateStep = () => {
        ...
        if (step === 3 && (!formData.age || !formData.qualification || !formData.occupation)) {
  ...
  onChangeText={(value) => {
    ...
    // Update the state only if the numeric value is 10 digits or less
    if (numericValue.length <= 10) {
      handleChange("mobileNumber", numericValue);
    }
  }}
  value={formData.mobileNumber}
  maxLength={10} // Add the maxLength prop
/>
    </>
)}
